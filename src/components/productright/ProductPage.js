...
function ProductPage() {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      if (!productId) return; // productId가 없으면 실행 안 함

      try {
        setReviewsLoading(true);
        setReviewsError(null);
        // 백엔드 ReviewController의 getReviewsByProductId API 호출
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/reviews/product/${productId}`);
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({ message: `리뷰 정보를 가져오는 중 오류가 발생했습니다. (상태: ${response.status})` }));
          throw new Error(errorData.message || `리뷰 정보를 가져오는 중 오류가 발생했습니다. (상태: ${response.status})`);
        }
        const data = await response.json(); // API 응답은 List<ReviewDTO.Response>
        setReviews(data);
      } catch (e) {
        setReviewsError(e);
        console.error("상품 리뷰 정보를 가져오는 중 오류 발생:", e);
      } finally {
    ...
    fetchReviews();
  }, [productId]); // productId가 변경될 때마다 리뷰 다시 로드
  ...
  return (
    <>
      ...
      {/* ProductImageGallery 컴포넌트 아래 */}
      <div className={styles.productReviewsSection}>
          <h2>상품 리뷰 ({reviews.length > 0 ? reviews.length : '0'})</h2>
          {reviewsLoading && <p>리뷰를 불러오는 중...</p>}
          {reviewsError && <p className={styles.errorMessage}>리뷰 로딩 오류: {reviewsError.message}</p>}
                ...
                <li key={review.id} className={styles.reviewItem}>
                  <div className={styles.reviewAuthorInfo}>
                    <span className={styles.reviewAuthorName}>
                      {/* UserDTO.Response에 profileImageUrl이 있다면 프로필 이미지도 표시 가능 */}
                      {review.user ? review.user.name : '익명 사용자'}
                    </span>
                  ...
                  <div className={styles.reviewRating}>
                    {/* 별점 표시 (예: 텍스트 또는 아이콘 사용) */}
                    {'★'.repeat(review.rating)}{'☆'.repeat(5 - review.rating)}
                  </div>
                  {review.reviewImageUrl && (
                    <img src={`${process.env.REACT_APP_API_BASE_URL}${review.reviewImageUrl}`} alt={`리뷰 이미지 ${review.id}`} className={styles.reviewImage} />
                  )}
